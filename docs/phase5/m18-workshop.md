...
!!! example "Ситуация"
    ...
    Ты data scientist в iGaming-продукте. К тебе приходит руководитель CRM / Head of Retention: команда готовит реактивационную рассылку с бонусом по базе зарегистрированных, но «спящих» игроков. Бюджет ограничен — бонус нельзя лить на всех, нужен скоринг «кто с высокой вероятностью внесёт депозит», чтобы бить точечно по верхушке списка.

    - Сейчас рассылку шлют веером по всей базе: конверсия в депозит ~15%, при ставке бонуса ~1000 ₽ на игрока и базе 20 000 контактов это ориентировочно 20 млн ₽ бонусного бюджета в месяц на 3 000 реальных депозитов — остальное уходит на тех, кто не платит.
    - От твоей модели зависит решение: на какую долю базы (топ-N по скору) реально лить бонус, чтобы при том же бюджете поднять долю «платящих» в выборке и не сжечь деньги на нерелевантных. Ошибка ранжирования = недополученные депозиты либо слитый бюджет.
    - Отдельный вопрос на столе: тащить ли в прод нейросеть (дороже в поддержке и инференсе) или хватит бустинга. Ответ должен быть подкреплён цифрой ROC-AUC/PR-AUC, а не «хочется попробовать deep learning».
    - Ограничение: есть только табличные признаки игрока и одна неделя на пилот, поэтому решение принимаем на отложенном test, без переусложнения.
...
Три числа ROC-AUC — это ещё не результат для CRM-руководителя. Переведи технику в решение по рассылке и бюджету.

- [ ] **Рекомендация.** Какую модель берём в прод для скоринга (логрег / MLP / LightGBM) и на какую долю базы (топ-N по скору) льём бонус. Формулируй как действие: «бьём бонусом по топ-30% скоринга», а не «AUC модели X = 0.84».
- [ ] **Эффект в деньгах/метриках.** Покажи, что даёт ранжирование: при том же бонусном бюджете доля платящих в выборке топ-N выше базовых 15%, значит на условные 20 млн ₽ собираем больше депозитов (или столько же депозитов дешевле). Привяжи к PR-AUC — на дисбалансе именно она отражает качество отбора редкого «платящего».
- [ ] **Риски и допущения.** Синтетика и offline-метрика не равны проду: скор на исторических признаках, концепт-дрифт поведения игроков, риск, что MLP-выигрыш в 0.00X AUC не окупает стоимость поддержки и инференса против LightGBM.
- [ ] **Следующий шаг.** Что предлагаешь дальше: A/B рассылки по скору против веерной, калибровку порога под бюджет, мониторинг конверсии топ-сегмента после запуска.
- [ ] **Как подать стейкхолдеру.** Язык решений: «эта модель позволяет при том же бюджете сфокусировать бонус и поднять долю реальных депозитов в рассылке; нейросеть прироста не дала — рекомендую LightGBM как дешевле в эксплуатации». Без терминов backprop/dropout — только что меняется в деньгах и в кампании.
