...
!!! example "Ситуация"
    ...
    Ты data scientist в iGaming-продукте. Head of Retention приходит с проблемой: отдел готовит CRM-кампанию по удержанию игроков из группы риска (бонусы, персональные офферы, обзвон VIP-саппортом), и нужна модель, которая заранее покажет, кто вот-вот отвалится. Бюджет кампании ограничен, цена ошибки в обе стороны разная, и от качества оценки модели напрямую зависит, на кого тратить деньги.

    - Контекст в цифрах (иллюстративно): отток в когортах около 8%, удержание одного игрока в среднем приносит ~8 000 ₽ будущего NGR, а один бонус-оффер стоит ~1 000 ₽. Поэтому пропустить уходящего (FN) дороже, чем зря угостить лояльного (FP), примерно в 8 раз.
    - Что на кону: если модель оценена нечестно (accuracy 0.97, утечка скейлера, random k-fold на временных данных), отдел поверит в красивую цифру, раскатает кампанию на весь сегмент и сольёт бюджет на ложные срабатывания, а реальный отток упустит. В проде модель окажется слабее обещанного, и доверие к аналитике рухнет.
    - От твоего ответа зависит: на сколько игроков и с каким порогом запускать кампанию, сколько бюджета бронировать, стоит ли вообще вкладываться в ML или хватит простого правила (baseline).
    - Ограничение: данных немного, классы перекошены, времени на R&D мало — нужен воспроизводимый каркас оценки, а не разовый прогон, которому нельзя верить.
...
Технический результат (PR-AUC, confusion matrix, подобранный порог) сам по себе ничего не решает — Head of Retention ждёт решение, а не метрику. Переведи цифры в язык денег и действий:

- [ ] **Рекомендация:** запускать кампанию по списку игроков, которых модель отметила с порогом из шага 4 (заметно ниже 0.5), а не по всему сегменту риска. Если содержательная модель не обгоняет baseline по PR-AUC — честно сказать, что ML здесь пока не нужен, хватит простого правила.
- [ ] **Эффект в деньгах:** из confusion matrix на test посчитай TP (пойманный отток × ~8 000 ₽ удержанного NGR) минус стоимость FP (ложные офферы × ~1 000 ₽) и FN (упущенные игроки). Покажи чистый ожидаемый эффект кампании и требуемый бюджет на офферы.
- [ ] **Риски и допущения:** цена ошибок (8:1) и стоимость оффера — оценочные; PR-AUC и порог имеют разброс (variance по фолдам/сидам); модель валидна, пока поведение игроков и сегменты стабильны — при дрейфе оценку надо пересматривать.
- [ ] **Следующий шаг:** запустить пилот на одной когорте с замером фактического удержания vs прогноз, перепроверить порог на свежих данных через 2-4 недели, и только потом масштабировать.
- [ ] **Как подать:** одним предложением в терминах решения — «при пороге X охватываем N игроков, ожидаемо удерживаем M из них, чистый эффект ~K ₽ при бюджете B ₽», а не «PR-AUC модели 0.34».
