...
!!! example "Ситуация"
    ...
    Ты data scientist в команде iGaming-продукта. К тебе приходит **Head of Retention**: за последние когорты отток на горизонте 14 дней пополз вверх, а CRM-команда сейчас шлёт удерживающие бонусы «по площади» — всем, кто давно не заходил. Это дорого и бьёт по марже.

    - Запущена ретеншн-кампания: персональный бонус на удержание стоит ориентировочно **800 ₽ на игрока**, бюджет на следующий месяц — **~1.5 млн ₽** (примерно на 1 900 игроков). Нужна модель, которая выдаст приоритетный список тех, кто реально вот-вот уйдёт, а не «всех подряд».
    - Текущая рассылка «по простою» ловит мало настоящих уходящих и жжёт бонусы на тех, кто и так остался бы. Каждый процент промаха по precision — это десятки тысяч рублей бонусов в молоко.
    - От твоего ответа зависит, **какую модель ставим в прод** и **под какой порог точности** настраиваем триггер кампании. Ключевая бизнес-метрика — `recall@precision≥0.6`: сколько настоящего оттока поймаем, держа точность тревог не ниже 60%, чтобы бонусный бюджет не утекал на ложные срабатывания.
    - Ограничение: данных на сложную ML-инфраструктуру нет, на руках только табличный снимок по игрокам, и решение про конфигурацию кампании нужно **к концу спринта** — без месяцев на эксперименты.
...
Технический результат (таблица метрик и важности) сам по себе Head of Retention ничего не говорит. Переведи его в решение про конфигурацию кампании. Подготовь короткий ответ на языке решений, а не метрик:

- [ ] **Рекомендация.** Какую модель ставим в прод как скоринг оттока и под какой порог настраиваем триггер кампании (например: «LightGBM на пороге, дающем precision ≥ 0.6»). Одно предложение, без «AUC выше».
- [ ] **Эффект в деньгах.** Переведи `recall@P0.6` в бизнес: при бюджете ~1.5 млн ₽ (≈1 900 бонусов по 800 ₽) модель выводит в приоритет N реально уходящих игроков против рассылки «по простою»; оцени, сколько бонусов перестаём тратить на тех, кто и так остался бы (рост precision → экономия бюджета), и сколько удержанной выручки добавляет пойманный отток.
- [ ] **Риски и допущения.** Метрики получены на случайном сплите и синтетике: на реальной поздней когорте они просядут; скоры не откалиброваны (порог под precision — не вероятность); драйверы оттока (простой, активность) могут сдвинуться при смене продукта или гео-микса.
- [ ] **Следующий шаг.** Что делаем до раскатки: временная (out-of-time) валидация на свежей когорте, калибровка вероятностей перед денежным решением, A/B запуск кампании на сегменте «high-risk» против контрольной группы.
- [ ] **Как подать стейкхолдеру.** Один слайд: «Можем поймать X% уходящих игроков, не раздувая бонусный бюджет; экономим ≈Y ₽ на ложных тревогах относительно текущей рассылки. Драйверы — простой и падение активности. Прошу две недели на out-of-time проверку и A/B перед полной раскаткой».
