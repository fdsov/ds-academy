...
Ты — data scientist в команде iGaming-продукта. К тебе приходит Head of Retention: отдел готовит квартальный бюджет на удержание и хочет таргетировать «горящие» рисковые когорты персональными бонусами и звонками VIP-менеджеров. Решение прямое — выделять деньги под список игроков, который выдаёт твоя churn-модель.

!!! example "Ситуация"

    - Прошлая версия модели на офлайн-валидации показывала AUC ~0.97, и на этой цифре отдел заложил план: обзвон и бонусы на топ-10% «рисковых» игроков, бюджет ориентировочно 1.5 млн ₽ в квартал, ожидаемое спасение когорты с месячной выручкой ~6 млн ₽.
    - В проде модель не сработала: реальный отклик оказался близок к случайному, бонусы ушли тем, кто и так бы остался, а уходящих не поймали. Деньги сожгли впустую.
    - От тебя нужен честный ответ на два вопроса: можно ли вообще верить этой модели и какой список игроков отдавать в обзвон. На кону повторное согласование бюджета и доверие к аналитике со стороны Head of Retention.
    - Ограничение: данные — недельные снимки активности игроков, у тебя 1-2 дня до защиты бюджета на ретеншн-комитете; новых данных собрать не успеешь, работаешь с тем, что есть.
...
Технический результат сам по себе Head of Retention не нужен — ему нужно решение по бюджету. Переведи цифры воркшопа в язык решений и подготовь короткий ответ для ретеншн-комитета.

- [ ] **Рекомендация.** Сформулируй прямо: «прошлой модели с AUC 0.97 верить нельзя — это утечка, реальная различающая сила ~0.70-0.78; список рисковых игроков надо строить по честной модели, а не по старой».
- [ ] **Эффект в деньгах.** Покажи цену ошибки таблицей «до/после»: разрыв ~0.2 AUC — это и есть переоценка, из-за которой бюджет 1.5 млн ₽/квартал ушёл бы в молоко. Оцени, сколько спасает корректный таргетинг через lift над baseline (сложность модели оправдана, только если прирост заметный).
- [ ] **Риски и допущения.** Честный AUC ~0.75 означает, что модель ранжирует, но не творит чудо: часть бюджета всё равно уйдёт на ложные срабатывания. Оговори, что SHAP — это объяснение поведения модели, а не каузальность: бонус за «низкую частоту ставок» не гарантированно удержит игрока.
- [ ] **Следующий шаг.** Предложи запустить удержание не вслепую, а как A/B на топ-децили честной модели (контроль без бонуса), чтобы измерить реальный uplift на деньгах прежде, чем масштабировать бюджет на весь квартал.
- [ ] **Как подать.** Один слайд на языке решений: «старая цифра завышена, реальный потолок такой-то, рекомендую бюджет под пилот на проверенной когорте, эффект подтверждаем экспериментом» — без слов AUC/SHAP/GroupKFold в основном тезисе, метрики оставь в приложении.
