...
Ты — продуктовый аналитик в iGaming-команде. Артефакт воркшопа — это не «графики ради графиков», а слайд, который кто-то понесёт на бюджетный комитет и по которому перераспределят живые деньги на закупку трафика.

!!! example "Ситуация"

    К тебе приходит **руководитель партнёрской программы** (отвечает за закупку трафика по каналам A и B). На ревью маркетинг-бюджета всплыло: канал A льёт объём, но что-то не сходится по экономике, а свежие когорты выглядят слабее старых. Решают, резать ли закупку по A.

    - **Проблема (иллюстративные цифры):** канал A приносит ~6 000 регистраций в месяц при CPA ≈ 9 у.е., но средний LTV D30 по A просел ниже стоимости привлечения (LTV < CPA) — то есть каждый игрок из A окупается в минус. При таком объёме это ориентировочно сотни тысяч у.е. в месяц, уходящих в убыточную закупку. Retention day-7 в свежих когортах A заметно ниже, чем в старых, — качество входящего трафика деградирует от недели к неделе.
    - **Что зависит от ответа:** сократить или полностью остановить закупку по A и переложить бюджет на B (где LTV > CPA) — либо оставить как есть, если убыток окажется артефактом графика. Цена ошибки в обе стороны — месячный бюджет канала.
    - **Ограничение:** на комитет идёшь через 2 дня, внешних данных нет — только выгрузка по игрокам (депозиты, LTV D30, канал, неделя регистрации) и лог возвратов. Решение нужно защитить честными осями, а не «красивой» картинкой, которая разваливается при первом вопросе.
...
Графики готовы — но руководителю партнёрской программы нужен не `story_final.png`, а решение. Переведи технический результат в язык денег и действий: на комитете звучит «режем закупку A», а не «у канала A ECDF левее».

- [ ] **Рекомендация:** одно предложение с глаголом и числом — например «сократить закупку по каналу A на горизонт следующего месяца, бюджет переложить на B». Не «LTV ниже», а что конкретно делать.
- [ ] **Эффект:** оцифруй в у.е./месяц — сколько убыточной закупки прекращаем (CPA − LTV) × объём регистраций A, и что даёт реаллокация на B (LTV > CPA). Покажи дельту по marketing-PnL, а не по retention-процентам.
- [ ] **Риски и допущения:** LTV D30 — горизонт 30 дней, часть отдачи A может прийти позже (предупреди про усечённый горизонт); CPA взяты как условные; деградация когорт может быть сезонной, а не структурной. Назови, что проверить, прежде чем резать «в ноль».
- [ ] **Следующий шаг:** что делаем после решения — фасетная heatmap по каналам как «причина» (см. ниже), пере-замер LTV на D60/D90, или пилот со срезанным на N% бюджетом A вместо полной остановки.
- [ ] **Как подать:** один слайд `story_final.png` с заголовком-выводом, два числа (убыток A, окупаемость B), один следующий шаг. Без жаргона «когорта/ECDF» — стейкхолдер должен принять решение за 30 секунд.
