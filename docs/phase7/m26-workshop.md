...
Этот воркшоп фундаментальный — ты строишь графики на синтетике, а не решаешь продуктовую задачу напрямую. Но навык, который он ставит, лежит под каждой моделью, которую ты выкатишь в прод. Поэтому вот зачем это в реальной работе iGaming.

!!! example "Ситуация"

    Ты — data scientist в команде iGaming-продукта. **Head of Retention** просит модель, которая по поведению игрока предсказывает риск оттока, чтобы адресно слать бонусы только тем, кто реально на грани ухода. Команда уже собрала первую версию: на исторических данных за март она показывает почти идеальное качество, и Head хочет завтра пустить её на весь трафик.

    - **Что на кону.** Бюджет на удержание — ориентировочно 3 млн ₽ в месяц на бонусы «рисковым» игрокам. Если модель переобучена (запомнила шум марта, а не выучила сигнал), на новых игроках она бьёт мимо: половина бонусов уходит не тем, кто уходит, — это порядка 1.5 млн ₽ в месяц на ветер плюс упущенное удержание тех, кого модель не разглядела.
    - **Что зависит от твоего ответа.** Запускать ли модель как есть, упростить ли её (контроль ёмкости, регуляризация), или собрать больше данных. «Идеальное качество на трейне» — это train-MSE из шага 1; разрыв с реальностью — это generalization gap, который ты в этом воркшопе учишься видеть и измерять руками.
    - **Ограничение.** Решение нужно за 2 недели, доступны только исторические логи за 3 месяца — переснять выборку «начисто» нельзя, поэтому оценивать обобщение приходится теми же приёмами (bootstrap, отложенный тест, кривые сложности), что и здесь.

    Воркшоп даёт тебе глаз на эту проблему: отличать модель, которая выучила закономерность, от модели, которая запомнила прошлое, — до того как стейкхолдер потратит на неё бюджет.
...
Графики и таблица — это технический результат. Стейкхолдеру (Head of Retention из бизнес-кейса) нужен не U-образный график, а решение: запускать модель или нет и что с бюджетом. Переведи то, что ты увидел на синтетике, в язык решений и собери короткий ответ по чек-листу.

- [ ] **Рекомендация.** Одна фраза, что делать с моделью оттока: запускать как есть / упростить через регуляризацию (подобрать $\lambda$ как в шаге 3) / собрать больше данных. Опирайся на разрыв train vs test — если он большой, модель переобучена и в прод в текущем виде идти не должна.
- [ ] **Эффект в деньгах или метриках.** Свяжи выбор сложности с бюджетом: «оптимальная сложность срезает промахи таргетинга примерно на X%, это около Y ₽ из 3 млн ₽ в месяц, которые перестанут уходить не тем игрокам». Цифру дай как оценку, не как факт.
- [ ] **Риски и допущения.** Назови главное: тест приближает реальный риск только если новые игроки i.i.d. с обучающими (а на игрока часто несколько строк — см. ловушку GroupKFold в шаге 1 «Развитие»); неустранимый шум $\sigma^2$ ставит потолок качества — ниже него не прыгнем никакой моделью.
- [ ] **Следующий шаг.** Конкретное действие: например «зафиксировать сложность на оптимуме по отложенному тесту, прогнать на апрельской когорте, сверить промахи таргетинга до раскатки на весь трафик».
- [ ] **Как подать стейкхолдеру.** Говори решениями, не метриками: не «test-MSE 0.11 при depth=5», а «модель уверенно ловит уходящих игроков и не путается на новых — бюджет на удержание тратится по адресу; запускаем на ограниченной когорте, через две недели сверяем».
