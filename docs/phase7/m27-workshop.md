...
Ты data scientist в команде risk & antifraud iGaming-продукта. Антифрод-лид прибежал к тебе после жалобы: служба поддержки фиксирует всплеск тикетов «заблокировали вывод без причины», и почти все они из одного региона. Твоя скоринг-модель автоматически тормозит выплаты при высоком скоре фрода — и есть подозрение, что она систематически жёстче к игрокам из этого региона, хотя сам регион в обучение не подавался. Юристы напоминают: автоматическая блокировка выплаты — это «значимое автоматическое решение» под ст. 22 GDPR и риск-категорию EU AI Act, а регулятор может запросить доказательство недискриминации.

!!! example "Ситуация"

    - Из ~20 000 игроков в скоринге доля автоматических отказов в выводе по региону north примерно вдвое выше, чем по south, при сопоставимом реальном уровне фрода — это похоже на дискриминацию через прокси, а не на разный риск.
    - Каждый ложный отказ легитимному игроку бьёт по retention: ориентировочно 300-400 ошибочных блокировок в месяц при среднем LTV игрока ~25 000 ₽ — это порядка 8-10 млн ₽ потенциально потерянной выручки и репутационный риск.
    - На кону решение: оставить модель в проде как есть, выкатить митигацию или вернуть часть кейсов на ручное ревью; плюс штрафной риск при проверке регулятором, если справедливость не задокументирована.
    - Ограничение: антифрод-лиду нужен вывод за 1-2 дня, доступны только обезличенные данные скоринга и квазиидентификаторы — прямых ID игроков у тебя нет.
...
Технический артефакт `fairness_audit.md` — это ещё не ответ для антифрод-лида. Переведи числа в решение на языке бизнеса и риска, а не метрик. Заполни чек-лист:

- [ ] Рекомендация: что делаем с моделью — оставить, выкатить митигацию порогом по группам или вернуть пограничные кейсы (скор в зоне разрыва) на ручное ревью. Одна фраза, понятная нетехническому стейкхолдеру.
- [ ] Эффект в деньгах и метриках: на сколько падает разрыв в доле отказов между регионами, сколько ложных блокировок в месяц это снимает и какой объём удержанной выручки (LTV × снятые блокировки) это даёт против цены — просадки агрегатной accuracy и роста пропущенного фрода.
- [ ] Риски и допущения: митигация выравнивает equalized odds, но осознанно жертвует калибровкой (теорема о невозможности); цифры на синтетике иллюстративны и требуют пересчёта на реальных логах; разрыв может вернуться при дрейфе данных.
- [ ] Следующий шаг: с какой даты включаем human-in-the-loop на блокировках выплат, кто владелец повторного аудита и с какой периодичностью; что показываем регулятору при запросе.
- [ ] Как подать стейкхолдеру: одна страница с выбором «оставить / митигировать / ручное ревью», ценой и риском каждого варианта — решение принимает антифрод-лид, твоя задача дать ему обоснованный выбор, а не таблицу метрик.
