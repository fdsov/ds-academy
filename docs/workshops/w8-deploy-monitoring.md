...
Технический результат (`action: RETRAIN`, таблица PSI/KS) сам по себе ничего не решает — Head of Retention не читает таблицы статистик. Переведи его в решение на языке бизнеса:

- [ ] **Рекомендация.** Одна фраза действия: «Модель поехала на новом гео-трафике — её скорам по топ-децилю риска временно нельзя доверять как раньше, запускаем переобучение на свежих данных и до его готовности корректируем рассылку бонусов».
- [ ] **Эффект в деньгах.** Свяжи дрейф с бюджетом: сдвиг `bonus_ratio` и recency означает, что часть бонусов (порядка ~0.5 млн ₽/мес по нашей оценке) уходит не той аудитории; переобучение возвращает точность таргетинга. Назови цифру, а не «PSI = 0.31».
- [ ] **Риски и допущения.** Дрейф входа — ранний сигнал, но он НЕ доказывает падение качества напрямую: финальное подтверждение даст фактический отток через 30 дней. Оценка потерь иллюстративная, привязана к текущему бонус-бюджету и доле топ-дециля. Пороги триггера (PSI ≥ 0.25) — стартовый дефолт, калибруется по истории.
- [ ] **Следующий шаг.** Конкретное действие с владельцем и сроком: переобучить и выкатить версию 1.1.0 до старта залива; на 30-дневном горизонте подключить мониторинг качества по ground truth (rolling AUC), чтобы ловить и концепт-дрейф, а не только дрейф входа.
