...
!!! info "Почему это важный навык"
    ...
    RAG — самый востребованный паттерн прикладного LLM в продакшене 2026 года. Он позволяет давать модели свежие и приватные знания без дообучения. Главная боль здесь не "подключить LLM", а **сделать ответы проверяемыми**: retrieval-качество и цитирование решают больше, чем выбор модели.
...
Модель `intfloat/multilingual-e5-base` хорошо работает с русским. У e5-моделей важная деталь: документы кодируются с префиксом `passage:`, запросы — с `query:`. Это часть их обучения, без префиксов качество заметно падает.
...
```bash
uv run hf download bartowski/Qwen2.5-3B-Instruct-GGUF \
...
def main():
    ...
    gen = LocalGenerator("models/Qwen2.5-3B-Instruct-Q4_K_M.gguf")
...
- **Эмбеддинг-модель не под язык корпуса.** Дефолтные `all-MiniLM` заточены под английский; на русском Hit Rate проседает. Бери многоязычную (e5, bge-m3). Симптом: retrieval промахивается даже на очевидных вопросах.
...
- **Реранкер.** Поставь cross-encoder (`bge-reranker-v2-m3`) после top-k: достаёшь top-20 быстрым поиском, переранжируешь 20 точной моделью, в контекст отдаёшь top-4. Заметно поднимает MRR.
