...
!!! example "Ситуация"
    ...
    Ты data scientist в iGaming-продукте. Churn-модель уже крутится в проде и каждое утро отдаёт CRM список «вот-вот уйдут» — по нему Head of Retention запускает реактивацию (бонусы, пуши, звонки VIP-менеджеров). Бюджет на удержание — ориентировочно 4 млн ₽/мес, и он распределяется ровно по тому, кому модель ставит высокий `churn_proba`.

    Неделю назад маркетинг залил большую кампанию на новую гео-аудиторию: средний депозит новичков подскочил с ~40 до ~60 у.е. Модель училась на старом мире и теперь по новой аудитории молча выдаёт мусорные вероятности. Head of Retention приходит с вопросом: «Можно ли вообще доверять утреннему списку, или мы жжём бюджет на удержание тех, кто и так бы остался?»

    - Что зависит от ответа: распределение тех самых 4 млн ₽/мес. Если модель поехала, реактивация бьёт не по тем — это и недоудержанные игроки (прямой минус к LTV), и впустую розданные бонусы.
    - Цена тихого сбоя: модель не падает с ошибкой, она уверенно врёт. Без детектора дрейфа никто не узнает, пока через месяц не просядет retention в когортах.
    - Ограничение: разметка (ушёл/остался) приедет только через 30 дней, ждать её нельзя. Решение нужно принять сейчас, по входным данным — отсюда мониторинг распределений (PSI/KS), а не метрик качества.
...
Технический результат (`deposits_30d` в ALERT, PSI `>=0.2`, KS p-value около нуля) сам по себе не нужен Head of Retention — ему нужно решение. Переведи отчёт детектора в одно сообщение на языке денег и действий.

- [ ] Рекомендация: одним предложением — что делать сейчас. Например: «Утренний churn-список по новой кампанийной аудитории не использовать до переобучения; по старой аудитории список валиден».
- [ ] Эффект в деньгах/метриках: к чему это привязано. «Под риском некорректного таргетинга — доля retention-бюджета (~X из 4 млн ₽/мес), приходящаяся на новую гео-когорту; цена ошибки — недоудержанные игроки и розданные впустую бонусы».
- [ ] Риски и допущения: дрейф признака ≠ деградация модели. Явно скажи, что сначала проверяешь ETL/источник (не сломался ли парсинг депозитов), и что вывод сделан по входным распределениям — прямого падения качества пока не видно, labels приедут через 30 дней.
- [ ] Следующий шаг: конкретно и с владельцем. «Переобучаю модель на свежем батче с новой аудиторией до конца недели; до этого момента для кампанийной когорты — ручное правило/прежний сегментный подход».
- [ ] Как подать: говори решениями, не метриками. Не «PSI 0.27 и KS p<0.001», а «модель ослепла на новой аудитории после кампании, доверять её спискам по ним нельзя — вот план и сроки». Цифры — в сноску для тех, кто спросит «почему».
