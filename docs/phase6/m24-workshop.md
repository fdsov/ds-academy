...
!!! example "Ситуация"
    ...
    Ты data scientist в команде iGaming-продукта. К тебе приходит Head of Retention: в когорте марта удержание на 30-й день просело, и он хочет запустить кампанию с удерживающим бонусом, но бюджет жёстко ограничен. Деньги дадут только под понятный возврат, и решить надо за неделю — до старта апрельской рассылки.

    - Вводные: бонус 5 ₽ на игрока, бюджет кампании 10 000 ₽, то есть охват максимум 2000 игроков из ~40 тыс. в когорте. ARPU 50 ₽/мес, маржа 50%, базовое удержание ~40%.
    - На кону два решения: (1) запускать ли кампанию вообще — то есть докажешь ли ты, что бонус реально удерживает, а не «и так бы остались»; (2) кого им накрыть — отобрать 2000 игроков по риску оттока (как привыкли в команде) или по инкрементальному эффекту. Ошибка в выборе таргетинга означает, что бюджет уйдёт на «безнадёжных» и лояльных, а удержим почти ноль.
    - Что зависит от ответа: пойдёт ли бюджет на масштабирование на всю базу в следующих месяцах, или кампанию свернут. Стейкхолдеру нужен не AUC модели, а цифра доп. NGR с диапазоном и явная альтернатива «сколько потеряли бы на наивной рассылке».
...
Технический артефакт готов — теперь переведи его в решение, которое Head of Retention сможет защитить перед финансами без единого слова про Brier и Qini. Собери ответ на языке решений:

- [ ] Рекомендация: одной строкой — запускать ли кампанию и как таргетить (uplift-отбор 2000 игроков вместо churn-ранжирования), с условием выката (A/B + holdout 3%, потом 100%).
- [ ] Эффект в деньгах: сколько игроков удержим сверх случайной раздачи и сколько это доп. NGR за кампанию — с диапазоном из бутстрап-ДИ, а не одной точкой.
- [ ] Альтернатива в деньгах: сколько удержал бы наивный churn-таргетинг при том же бюджете — разница и есть цена правильного решения.
- [ ] Риски и допущения: эффект подтверждён на симуляции, а не на проде; guardrails (отток лояльных / sleeping dogs, доля жалоб, latency скоринга); порог чувствителен к стоимости бонуса.
- [ ] Следующий шаг и подача: запросить holdout-слот под A/B, дать стейкхолдеру цифру доп. NGR и точку отката (shadow mode), не вываливая метрики модели — он принимает решение по деньгам и риску.
