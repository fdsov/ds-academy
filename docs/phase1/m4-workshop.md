...
- [ ] Эффект в деньгах и метриках: на сколько процентных пунктов отличается D7 между лучшим и худшим каналом, какая доля выручки приходится на whale-сегмент (Шаг 5) — и что это значит для перераспределения 1,5 млн ₽/мес и решения по VIP-менеджеру.
