...
!!! example "Ситуация"
    ...
    Ты заглядываешь в сырьё и видишь типичный «партнёрский CSV»: суммы строкой с запятой и пробелами, часть строк в битой кодировке, депозиты с пропущенной суммой, события, датированные 2099 годом, и дубли от ретраев загрузки. Плюс десяток транзакций по 10 000 000,00 — явно тестовые прогоны, которые раздувают оборот.
    ...
    - **Цена ошибки:** один процент фантомного оборота на этих объёмах — это около 3 млн ₽ некорректной строки в отчёте; тестовые транзакции и дубли вместе дают как раз порядок наблюдаемого расхождения.
...
- [ ] **Рекомендация:** заявить, что для отчётности используется silver-таблица, а не сырой партнёрский CSV; источник истины по обороту — очищенный слой с зафиксированным контрактом качества.
- [ ] **Эффект в деньгах:** показать, на сколько упал депозитный оборот после чистки и из чего сложилось расхождение — отдельно дубли-ретраи (≈2% строк), отдельно тестовые транзакции по 10 млн, отдельно будущие даты. Свести к одной фразе: «после чистки оборот совпал с эквайрингом с точностью до X%».
- [ ] **Риски и допущения:** честно сказать, что пропущенные суммы депозитов (MAR) не удалены и не выдуманы, а помечены флагом — полнота осталась прежней осознанно; завышать её означало бы внести ложь в P&L. Оговорить, что винзоризация гасит влияние крупных сумм, но не различает хайроллера и тест.
- [ ] **Следующий шаг:** предложить поставить контракт `BetsSchema` на ежедневную выгрузку, чтобы битый файл от партнёра падал на входе и попадал в алерт, а не в отчёт через неделю.
- [ ] **Как подать:** одно предложение для совета — «оборот за период составляет 305 млн ₽, расхождение с банком устранено, источник — контролируемый пайплайн», а не «мы починили дубли и распарсили amount». Стейкхолдер покупает надёжность цифры, а не описание чистки.
