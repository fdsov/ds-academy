...
!!! example "Ситуация"
    ...
    Твоя задача в этом воркшопе — закрыть саму причину расхождений: собрать проект так, чтобы тот же расчёт у Head of Retention, у коллеги и в CI давал идентичный результат с трёх команд. Без этого фундамента любая последующая модель удержания — это число, которое нельзя защитить.
...
- [ ] Эффект в деньгах: устранён риск увести ~1 200 000 ₽/квартал не в тот канал из-за нестабильной цифры; решения по push vs VIP-обзвон теперь опираются на один и тот же результат.
- [ ] Риски и допущения: число честно ровно настолько, насколько корректны сами признаки — здесь закрыта утечка будущего (cutoff) и фиксация версий/seed, но не проверена бизнес-валидность RFM-порогов; данные пока синтетические, на проде нужна та же дисциплина с реальной выгрузкой.
...
- [ ] Как подать: показывай решение и его цену, а не инструменты — «цифре можно доверять, вот сколько денег это защищает», а `pyproject.toml`/`uv.lock`/seed оставь как доказательство в приложении, не как суть сообщения.
